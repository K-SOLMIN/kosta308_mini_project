...
public class ManagerReservationView {
  ...
  public void managerReservationMenu() {
    while (true) {
      ...
      System.out.println("──────────────────────────────────────────────────────────────────────────────────");
      System.out.println(" 1.예약 승인/반려 || 2.예약 강제 취소 || 3.예약/반납 이력 조회 || 0.뒤로 가기");
      System.out.println("──────────────────────────────────────────────────────────────────────────────────");
  ...
  private void showHistoryFlow() {
    ...
    List<Reservation> list = reservationService.getReservationHistory();
    ...
    // 번호(4) 예약자(8) 신청날짜(12) 예약날짜(12) 교시(10) 구분(4) 시설/비품명(16) 상태(10) = 83
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      String createdAtStr = r.getCreatedAt() != null ? r.getCreatedAt().format(DT_FMT) : "-";
      ...
      if (r.getReturnedAt() != null) {
        String returnDateStr = r.getReturnedAt().format(DT_FMT);
        String condition = (r.getReturnCondition() != null && !r.getReturnCondition().isEmpty())
            ? r.getReturnCondition() : "이상 없음";
        System.out.println("     └ 반납: " + returnDateStr + "  /  " + condition);
      }
      ...
      if (r.getReason() != null && !r.getReason().isEmpty()) {
        String label;
        switch (r.getStatus()) {
          case "거절": label = "반려 사유"; break;
          case "취소": label = "취소 사유"; break;
          default:     label = "사유";      break;
        }
        System.out.println("     └ " + label + ": " + r.getReason());
      }
